...
describe("executeTasks – no-resolution mutations (batch Phase 2 path)", () => {
  it("succeeds for two no-resolution steps batched in a single mutation", async () => {
    const githubClient = {
      ...
      queryRaw: async <TData>(doc: unknown) => {
        const query = String(doc)
        // Both mutations are batched into one queryRaw call
        expect(query).toContain("closeIssue")
        expect(query).toContain("addComment")
        return {
          data: {
            step0: {
