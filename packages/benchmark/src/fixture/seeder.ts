...
async function buildManifest(
  ...
  if (needs("pr_with_reviews")) {
    if (reviewerToken) {
      try {
        prWithReviews = createPrWithReviews(
          repo,
          seedId,
          seedLabel,
          reviewerToken,
        ...
        console.warn(
          `warning: unable to seed pr_with_reviews fixture (${message})`,
        );
      }
    } else {
      console.warn(
  ...
  if (needs("pr_with_mixed_threads")) {
    if (reviewerToken) {
      try {
        prWithMixedThreads = createPrWithMixedThreads(
          repo,
          seedId,
          seedLabel,
          reviewerToken,
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.warn(
          `warning: unable to seed pr_with_mixed_threads fixture (${message})`,
        );
      }
    } else {
      console.warn(
