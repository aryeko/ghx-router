...
export function renderWorkflowPrompt(scenario: WorkflowScenario, mode: BenchmarkMode): string {
  const modeHint =
    mode === "ghx"
      ? "You MUST use `ghx run <task> --input '<json>'` commands to interact with GitHub. Do not use raw `gh` CLI or API calls. Run `ghx capabilities` if you need to discover available tasks."
      : mode === "agent_direct"
