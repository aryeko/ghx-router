...
- `pnpm run ghx:gql:check` – enforce generated GraphQL artifacts are in sync (offline check)
- `pnpm --filter @ghx-dev/core run gql:schema:update` – refresh `packages/core/src/gql/schema.graphql` from GitHub GraphQL (on-demand; requires `GITHUB_TOKEN` or `GH_TOKEN`)
